...
Feature selection in censored-covariate regression models, as described in:

> Shankar R, Garcia T, Ormerod J, Tarr G (2026). "Feature Selection in Censored-Covariate Regression Models." *Statistics in Medicine*.

The package fits a penalised censored-covariate regression model using an EM algorithm with two separate L1 penalties — one for features associated with the response variable and one for features associated with the censored covariate. Tuning parameters are selected via cross-validation.
